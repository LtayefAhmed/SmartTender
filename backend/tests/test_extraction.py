...
def _pdf_with_text(lines: list[str]) -> bytes:
    ...
    Built by hand on purpose. Generating it with a library would make these
    assertions depend on whichever version of that library happens to be
    installed — and the suite's whole premise is that it needs nothing.
    """
